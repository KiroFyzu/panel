    ...
    <div class="row equal-height">
        ...
        <div class="col-sm-4">
            <div class="box box-info">
                <div class="box-header with-border">
                    <h3 class="box-title">Server Expiration</h3>
                </div>
                <form action="{{ route('admin.servers.view.manage.toggle', $server->id) }}" method="POST">
                    <div class="box-body">
                        <p>Set when this server should expire. Expired servers are automatically suspended by the scheduled task.</p>
                        <div class="form-group">
                            ...
                            <input
                                ...
                                name="expires_at"
                                class="form-control"
                                value="{{ old('expires_at', $server->expires_at ? $server->expires_at->format('Y-m-d\TH:i') : '') }}"
                            />
                            <p class="small text-muted no-margin">Leave blank to remove the expiration date.</p>
                        </div>
                        @if($server->expires_at)
                            <p class="small no-margin">
                                Current expiration:
                                <strong>{{ $server->expires_at->toDayDateTimeString() }}</strong>
                        ...
                        @else
                            <p class="small no-margin"><span class="label label-default">No Expiration</span></p>
                        @endif
                    </div>
                    <div class="box-footer">
                        {!! csrf_field() !!}
                        <input type="hidden" name="update_expiration" value="1" />
                        <button type="submit" class="btn btn-info">Save Expiration</button>
                    </div>
                </form>
            </div>
        </div>
